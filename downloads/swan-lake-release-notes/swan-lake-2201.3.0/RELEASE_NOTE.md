...
Note that this change has affected the UUID module. Therefore, if an application uses the UUID module or uses a package which has a dependency on the UUID module, make sure that the UUID version is updated to 1.4.0. Otherwise, it would result in the aforementioned compilation error.
