...
Annotations for tuple members are now supported. Tuple members can be annotated using annotations with `field` attachment points. Similarly to records, annotations are not allowed on the `rest` type of a tuple
