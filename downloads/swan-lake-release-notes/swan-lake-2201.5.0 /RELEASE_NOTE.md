...
- Fixed a bug (in dependently-typed function analysis) that previously resulted in compilation errors not being logged, when the `typedesc` argument is defined using a type definition (`T`) and the return type is a union (`T|t`) (where the basic types for `T` and `t` are not disjoint).
