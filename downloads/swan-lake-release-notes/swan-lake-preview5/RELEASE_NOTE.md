...
Object field scope and object method scope are unified into a single object scope. This means that it's no longer possible to have fields and methods with the same name.

With this change, object methods can be accessed as object fields and the reference to `self` is captured in the resulting function pointer.
