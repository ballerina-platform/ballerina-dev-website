...
>**Note:** The `Cloud.toml` file is completely optional and you only have to specify the values you need to override. If the value is not specified for a certain field, the compiler will take the default value. All the supported properties can be found [below](#properties-of-the-cloudtoml-file).
...
4. Create a file named `Cloud.toml` in the package directory and add the content below.
    ...
    ***Cloud.toml***
    ...
    [settings]
    buildImage=true # you can make this to false if you don't want to build the image and you just need the dockerfile
    ```
...
>**Note:** Before invoking the container, let’s observe the dockerfile below that has been generated. This is a dockerfile created by the compiler extension from your code to run your http service in docker environment easily.
...
COPY anjana-hello_docker-0.1.0.jar /home/ballerina/jars/ 
...
...
COPY anjana-hello_docker-0.1.0.jar /home/ballerina

EXPOSE  9090
USER ballerina

CMD java -Xdiag -cp "anjana-hello_docker-0.1.0.jar:jars/*" 'anjana/hello_docker/0/$_init' || cat ballerina-internal.log
```
...
### Creating the Ballerina Package
In this sample, we are gonna write a ballerina application which reads a greeting string from a config map and greets the user upon http request. We are going to make a kubernetes deployment that has container resource limits, horizontal pod autoscaling, config maps and liveness , readiness probes with the help of code to cloud.
...
5. Create a file named `Cloud.toml` in the package directory and add the content below. You might remember some of these properties from docker sample. We can use the same properties since `cloud="k8s"` option builds both docker image and kubernetes. You can see a brief description of the properties in comments.
...
Execute the `bal build` command to build the Ballerina package and you view the output below. If you haven't added cloud option to `Ballerina.toml` file, you can execute `bal build --cloud=k8s` to generate the artifacts and build the package by proving the build option inline. 

```bash
$> bal build
Compiling source
        anjana/hello_k8s:0.1.0
...
Generating artifacts...
        ...
        kubectl apply -f /home/anjana/c2c-guide/hello_k8s/target/kubernetes/hello_k8s
...
>**Note:** Before invoking the kubernetes service, let’s observe the kubernetes yamls that has been generated. You should be able to find services for ports that has been exposed, HorizontalPodAutoscaler for scaling and Deployment and Config maps.
...
1. Execute the `kubectl apply -f target/kubernetes/hello_k8s` command to execute the service.
...
3. Execute the `kubectl expose deployment hello-k8s-deployment --type=NodePort --name=hello-k8s-svc-local` command to expose the service via NodePort to test in the developer environment.
...
4. Execute the `kubectl get svc` command to get the EXTERNAL-IP and port of the Kubernetes service.
...
5. Execute the `curl http://192.168.49.2:30342/helloWorld/sayHello` command to access the deployed service via cURL.
