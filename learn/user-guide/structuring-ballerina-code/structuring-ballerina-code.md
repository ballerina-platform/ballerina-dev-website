...
2. In the CLI, execute the `bal new helloworld` command to create a new Ballerina package.
    ...
    **The `Ballerina.toml` File**
    ...
    Above `Ballerina.toml` represents an application package. The organization, name, and the version of the package are set with default values when building or running the package.
    **The `main.bal` File**
