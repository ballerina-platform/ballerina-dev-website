...
var traverseFolder = function (dir) {
  ...
  list.forEach(function (file) {
    ...
    if (stat && stat.isDirectory()) {
      ...
      /* Is a file */
      filex = filex.replace(/swan-lake\/healthcare\/fhir-support\//g, "");
      results.push(filex);
    }
  });
  return results;
...
export async function getStaticPaths() {
  ...
  const files = traverseFolder("swan-lake/healthcare/fhir-support");
...
export async function getStaticProps({ params: { slug } }) {
  ...
  const fileName = fs.readFileSync(
    `swan-lake/healthcare/fhir-support/${slug}.md`,
    "utf-8"
  );
          ...
          <Container>
            <div className="topRow">
              ...
              <Col xs={1} className="gitIcon">
                <a
                  href={`${process.env.gitHubPath}swan-lake/healthcare/fhir-support/${slug}.md`}
                  target="_blank"
                  rel="noreferrer"
