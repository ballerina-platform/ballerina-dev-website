...
var traverseFolder = function (dir) {
  ...
  list.forEach(function (file) {
    ...
    if (stat && stat.isDirectory()) {
      ...
      /* Is a file */
      filex = filex.replace(/swan-lake\/integration-technology-tools\//g, "");
      results.push(filex);
    }
  });
  return results;
...
export async function getStaticPaths() {
  ...
  const files = traverseFolder("swan-lake/integration-technology-tools");
...
export async function getStaticProps({ params: { slug } }) {
  ...
  const fileName = fs.readFileSync(
    `swan-lake/integration-technology-tools/${slug}.md`,
    "utf-8"
  );
      ...
      <Layout>
        <Col sm={3} xxl={2} className="leftNav d-none d-sm-block">
          <LeftNav
            launcher="learn"
            id={id}
            mainDir="integration-technology-tools"
            sub={sub}
            third={third}
            Toc={LearnToc}
          />
          ...
          <Container>
            <div className="topRow">
              ...
              <Col xs={1} className="gitIcon">
                <a
                  href={`${process.env.gitHubPath}swan-lake/integration-technology-tools/${slug}.md`}
                  target="_blank"
                  rel="noreferrer"
