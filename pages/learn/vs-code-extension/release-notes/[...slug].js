...
var traverseFolder = function (dir) {
  ...
  list.forEach(function (file) {
    ...
    if (stat && stat.isDirectory()) {
      ...
      /* Is a file */
      filex = filex.replace(/swan-lake\/vs-code-extension\/release-notes\//g, "");
      results.push(filex);
    }
  });
  return results;
...
export async function getStaticPaths() {
  ...
  const files = traverseFolder("swan-lake/vs-code-extension/release-notes");
...
export async function getStaticProps({ params: { slug } }) {
  ...
  const fileName = fs.readFileSync(
    `swan-lake/vs-code-extension/release-notes/${slug}.md`,
    "utf-8"
  );
          ...
          <Container>
            <div className="topRow">
              ...
              <Col xs={1} className="gitIcon">
                <a
                  href={`${process.env.gitHubPath}swan-lake/vs-code-extension/release-notes/${slug}.md`}
                  target="_blank"
                  rel="noreferrer"
