...
var traverseFolder = function (dir) {
  ...
  list.forEach(function (file) {
    ...
    if (stat && stat.isDirectory()) {
      ...
      /* Is a file */
      filex = filex.replace(/swan-lake\/vs-code-extension\/try-the-services\//g, "");
      results.push(filex);
    }
  });
  return results;
...
export async function getStaticPaths() {
  ...
  const files = traverseFolder("swan-lake/vs-code-extension/try-the-services");
...
export async function getStaticProps({ params: { slug } }) {
  ...
  const fileName = fs.readFileSync(
    `swan-lake/vs-code-extension/try-the-services/${slug}.md`,
    "utf-8"
  );
