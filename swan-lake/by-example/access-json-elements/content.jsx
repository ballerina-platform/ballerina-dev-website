  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      ...
      <p>
        ...
        rules are less strict. <code>json</code> is defined to be a lax type
        along with any <code>map&lt;T&gt;</code> where <code>T</code> is a lax
        type.
      </p>

      <p>
        For example, field access (<code>.</code>) and optional field access (
        <code>?.</code>), which are generally allowed on records and objects for
        fields that are defined in the type descriptors, are also allowed
        additionally on lax types. For such operations, some of the type
        checkings are moved from compile time to runtime.
      </p>

      <p>
        The best way of accessing JSON elements is to convert the{" "}
        <code>json</code> value to a user-defined type.
      </p>

      <p>
        <code>check Expr</code> is treated as{" "}
        <code>check val:ensureType(Expr, s)</code> when the <code>Expr</code> is
        a subtype of JSON and the expected type is a subtype of{" "}
        <code>()|boolean|int|float|decimal|string</code>. <code>s</code> is a
        typedesc value representing the expected type.
      </p>
      ...
      >
        <Col className="d-flex align-items-start" sm={12}>
          <button
            ...
            onClick={() => {
              window.open(
                "vscode://wso2.ballerina/open-file?repoFileUrl=https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.8.2/examples/access-json-elements/access_json_elements.md",
                "_blank",
              );
            }}
            target="_blank"
          ...
          <button
            ...
            onClick={() => {
              window.open(
                "https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.8.2/examples/access-json-elements",
                "_blank",
              );
            }}
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run access_json_elements.bal`}</span>
