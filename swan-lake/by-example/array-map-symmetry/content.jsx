...
<p>Ballerina provides a fine symmetry between lists and mappings. Lists are indexed by integers and mappings are indexed by strings. In terms of the JSON format, lists are represented by arrays and mappings are represented by objects.</p>


<p>The constructor for lists uses square brackets and the constructor for mappings uses curly brackets. Both list and mappings types can be described in two ways using uniform member types and per-index member types. A list with a uniform  <code>T</code> member type is an array declared as <code>T[]</code>. A mapping with a uniform <code>T</code> type member is a map declared as <code>map&lt;T&gt;</code>. Similarly, a list with per-index member types <code>T0</code> and <code>T1</code> is a tuple, declared as <code>[T0, T1]</code>, and a mapping with per-index member types is a record declared as <code>record &#123; Tx x; Ty y; &#125;</code>. You can have an open type using the <code>...</code> notation. In the case of lists, an open type is declared as a tuple type as <code>[T0, Tr...]</code>. In the case of mappings, it is a record type declared as <code>record &#123;| Tx x; Tr...; |&#125;</code>.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/array-map-symmetry", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run array_map_symmetry.bal`}</span>
<span>{`foo`}</span>
<span>{`1`}</span>
