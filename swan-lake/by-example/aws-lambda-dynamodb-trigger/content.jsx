...
export const codeSnippetData = [`import ballerina/io;
...
<p>AWS Lambda is an event driven, serverless computing platform. Ballerina functions can be deployed in AWS Lambda by annotating a Ballerina function with <code>@awslambda:Function</code>, which should have the <code>function (awslambda:Context, json|EventType) returns json|error</code> function signature. For more information, see the <a href="/learn/run-in-the-cloud/function-as-a-service/aws-lambda/">AWS Lambda learn guide</a>.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal new aws-lambda-dynamodb-trigger`}</span>
</code></pre>
          </Col>
        </Row>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref3}>
      <code className="d-flex flex-column"><span>{`\$ aws lambda create-function --function-name notifysDynamoDB --zip-file fileb://aws-ballerina-lambda-functions.zip --handler aws-lambda-dynamodb-trigger.notifyDynamoDB --runtime provided --role arn:aws:iam::908363916111:role/lambda-role--layers arn:aws:lambda:us-west-1:134633749276:layer:ballerina-jre11:6 --memory-size 512 --timeout 10`}</span>
</code></pre>
          </Col>
        </Row>
...
<p>To invoke this function, create a DynamoDB table.</p>

<ul style={{ marginLeft: "0px" 
                      }} >
                        <li>
                            <span>1.</span>
                            <span>Go to &lt;a href=&quot;https://console.aws.amazon.com/iamv2/home#/roles&quot; target=&quot;_blank&quot;&gt;<strong>roles</strong>&lt;/a&gt;, and add <code>AWSLambdaDynamoDBExecutionRole</code> to the created role in the prerequisites.</span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} >
                        <li>
                            <span>2.</span>
                            <span>Go to the &lt;a href=&quot;https://us-west-1.console.aws.amazon.com/dynamodbv2&quot; target=&quot;_blank&quot;&gt;DynamoDB&lt;/a&gt;.</span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} >
                        <li>
                            <span>3.</span>
                            <span>Click <strong>Create Table</strong>, enter the table name, partition key, and create the table (If you already have a table created, you can skip this step).</span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} >
                        <li>
                            <span>4.</span>
                            <span>Click on the DynamoDB table, and then click the <strong>Exports and streams</strong> tab.</span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} >
                        <li>
                            <span>5.</span>
                            <span>Click <strong>enable DynamoDB stream details</strong>, and select the key attributes only for the event type.</span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} >
                        <li>
                            <span>6.</span>
                            <span>Once it's enabled, click <strong>Create a trigger</strong>, select the <code>notifyDynamoDB</code> from the dropdown, and create a trigger.</span>
                        </li>
                    </ul>

<p>Now, add an entry to the DynamoDB table to invoke the Lambda function. For this, go to &lt;a href=&quot;https://us-west-1.console.aws.amazon.com/dynamodbv2&quot; target=&quot;_blank&quot;&gt;<strong>Items</strong>&lt;/a&gt; in the DynamoDB, select the table, and click <strong>Create item</strong>. Once the item is entered into the table, go to the Lambda function, and check the logs via CloudWatch to see the object identifier in the logs.</p>
...
<span>{`\$ aws lambda invoke --function-name notifysDynamoDB --payload fileb://input.json echo-response.txt`}</span>
<span>{`{`}</span>
