...
export function AwsLambdaHelloWorld({codeSnippets}) {
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal new aws-lambda-hello-world`}</span>
</code></pre>
          </Col>
        </Row>
...
<span>{`        wso2/aws-lambda-hello-world:0.1.0`}</span>
<span>{`
`}</span>
<span>{`Generating executable`}</span>
<span>{`        @awslambda:Function: echo`}</span>
<span>{`
`}</span>
<span>{`        Run the following command to deploy each Ballerina AWS Lambda function:`}</span>
<span>{`        aws lambda create-function --function-name \$FUNCTION_NAME --zip-file fileb://<project-dir>/aws-lambda-hello-world/target/bin/aws-ballerina-lambda-functions.zip --handler aws-lambda-hello-world.\$FUNCTION_NAME --runtime provided --role \$LAMBDA_ROLE_ARN --layers arn:aws:lambda:\$REGION_ID:134633749276:layer:ballerina-jre11:6 --memory-size 512 --timeout 10`}</span>
<span>{`
`}</span>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref3}>
      <code className="d-flex flex-column"><span>{`\$ aws lambda create-function --function-name echo --zip-file fileb://aws-ballerina-lambda-functions.zip --handler aws-lambda-hello-world.echo --runtime provided --role arn:aws:iam::908363916111:role/lambda-role--layers arn:aws:lambda:us-west-1:134633749276:layer:ballerina-jre11:6 --memory-size 512 --timeout 10`}</span>
</code></pre>
          </Col>
        </Row>
