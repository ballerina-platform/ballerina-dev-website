...
export function AwsLambdaS3Trigger({codeSnippets}) {
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal new aws-lambda-s3-trigger`}</span>
</code></pre>
          </Col>
        </Row>
...
<span>{`        wso2/aws-lambda-s3-trigger:0.1.0`}</span>
<span>{`
`}</span>
<span>{`Generating executable`}</span>
<span>{`        @awslambda:Function: notifys3`}</span>
<span>{`
`}</span>
<span>{`        Run the following command to deploy each Ballerina AWS Lambda function:`}</span>
<span>{`        aws lambda create-function --function-name \$FUNCTION_NAME --zip-file fileb://<project-dir>/aws-lambda-s3-trigger/target/bin/aws-ballerina-lambda-functions.zip --handler aws-lambda-s3-trigger.\$FUNCTION_NAME --runtime provided --role \$LAMBDA_ROLE_ARN --layers arn:aws:lambda:\$REGION_ID:134633749276:layer:ballerina-jre11:6 --memory-size 512 --timeout 10`}</span>
<span>{`
`}</span>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref3}>
      <code className="d-flex flex-column"><span>{`\$ aws lambda create-function --function-name notifys3 --zip-file fileb://aws-ballerina-lambda-functions.zip --handler aws-lambda-s3-trigger.notifys3 --runtime provided --role arn:aws:iam::908363916111:role/lambda-role--layers arn:aws:lambda:us-west-1:134633749276:layer:ballerina-jre11:6 --memory-size 512 --timeout 10`}</span>
</code></pre>
          </Col>
        </Row>
...
<ul style={{ marginLeft: "0px" 
                        ...
                        <li>
                            <span>1.</span>
                            <span>Go to &lt;a href=&quot;https://s3.console.aws.amazon.com/s3/&quot; target=&quot;_blank&quot;&gt;AWS S3&lt;/a&gt; portal and create a bucket.</span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} >
                        <li>
                            <span>2.</span>
                            <span>Click on the created bucket, go to the <strong>Properties</strong> tab, and click on the <strong>Create event notification</strong> under the <strong>Event notifications</strong> section.</span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} >
                        <li>
                            <span>3.</span>
                            <span>Enable <code>All object create events</code> under event types. Select the Lambda function as the destination, and choose the <code>notifyS3</code> Lambda function from the dropdown.</span>
                        </li>
                    </ul>

<p>Now, click <strong>Upload</strong> to upload an object to the S3 bucket, and view the Lambda logs via CloudWatch to see the object name.</p>
