...
export function AzureFunctionsHelloWorld({ codeSnippets }) {
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      ...
      <p>
        This example demonstrates how to write a simple echo function in Azure
        Functions.
      </p>

      <p>
        In Ballerina, triggers are represented by listeners. When the{" "}
        <code>af:HttpListener</code> gets attached to the service, it implies
        that the function is an HTTP Trigger. The resource method behaves
        exactly the same as a service written from <code>ballerina/http</code>.
        It supports the <code>http:Payload</code> and <code>http:Header</code>{" "}
        annotations for parameters. Input binding annotations can be used to
        annotate parameters to make use of external services in Azure. If no
        annotations are specified for a parameter, it is identified as a query
        parameter.
      </p>

      <p>
        Output bindings are defined in the return type definition. For services
        with the <code>HttpListener</code> attachment, <code>HttpOutput</code>{" "}
        is the default output binding. You can override the default behavior by
        specifying them explicitly in the return type.
      </p>

      <p>
        In the code sample shown above, it has an empty service path and
        resource path named <code>hello</code>. The accessor is <code>get</code>
        . It expects a request with a query parameter for the field name. The
        required artifact generation and data binding will be handled by the{" "}
        <code>ballerinax/azure_functions</code> package automatically.
      </p>
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal new azure_functions_hello_world`}</span>
              <span>{`Created new package 'azure_functions_hello_world' at /Users/wso2/azure_functions_hello_world.`}</span>
            </code>
          </pre>
        </Col>
      </Row>
      ...
      >
        <Col className="d-flex align-items-start" sm={12}>
          <button
            ...
            onClick={() => {
              window.open(
                "https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.12.10/examples/azure-functions-hello-world",
                "_blank",
              );
            }}
        ...
        <Col sm={12}>
          <pre ref={ref2}>
            <code className="d-flex flex-column">
              ...
              <span>{`	wso2/azure_functions_hello_world:0.1.0`}</span>
              <span>{`
`}</span>
              <span>{`Generating executable`}</span>
              <span>{`
`}</span>
              <span>{`	@azure.functions:Function: get-hello`}</span>
              <span>{`
`}</span>
...
`}</span>
              ...
              <span>{`
`}</span>
              <span>{`	target/bin/azure_functions_hello_world.jar`}</span>
            </code>
          </pre>
        </Col>
      </Row>
        ...
        <Col sm={12}>
          <pre ref={ref3}>
            <code className="d-flex flex-column">
              ...
              <span>{`Uploading 32.69 MB [##############################################################################]`}</span>
              <span>{`Upload completed successfully.`}</span>
              <span>{`Deployment completed successfully.`}</span>
              <span>{`Syncing triggers...`}</span>
              <span>{`Functions in <function_app_name>:`}</span>
              <span>{`    get-hello - [httpTrigger]`}</span>
              <span>{`        Invoke url: https://azurecosmosdbfunctionapp.azurewebsites.net/hello`}</span>
            </code>
          </pre>
        </Col>
      </Row>
      ...
      <p>
        Execute the command below to invoke the function by replacing{" "}
        <code>&lt;function_app_name&gt;</code> with your respective function app
        name.
      </p>
      ...
      >
        ...
        <Col sm={12}>
          <pre ref={ref4}>
            <code className="d-flex flex-column">
              <span>{`\$ curl https://<function_app_name>.azurewebsites.net/hello\\?name\\=Jack`}</span>
