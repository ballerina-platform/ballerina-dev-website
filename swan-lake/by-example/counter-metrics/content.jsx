...
export function CounterMetrics({codeSnippets}) {
...
<p>Ballerina supports Observability out of the box and Metrics is one of the three important aspects of  Observability.To observe Ballerina code, the <code>--observability-included</code> build time flag should be given along with the <code>Config.toml</code> file when starting the service. The <code>Config.toml</code> file contains the required runtime configurations related to observability.</p>


<p>You can define and use metrics to measure your own logic. A counter is one type of the metrics that is supported by default in Ballerina, and it is a cumulative metric that represents a single monotonically-increasing counter whose value can only increase or be reset to zero.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/counter-metrics", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref2}>
      <code className="d-flex flex-column"><span>{`\$ BAL_CONFIG_FILES=Config.toml bal run --observability-included counter_metrics.bal`}</span>
