...
export const codeSnippetData = [`import ballerina/file;
...
export function DirectoryListener({codeSnippets}) {
...
<p>The <code>file</code> library provides a <code>Directory Listener</code>, which is used to listen to the given directory in the local file system.</p>

<p>It notifies when new files are created in the directory or when the existing files are deleted or modified.</p>


<p>For more information on the underlying module, see the <a href="https://lib.ballerina.io/ballerina/file/latest/"><code>file</code> module</a>.</p>


<p>Before running this, change the value of the <code>path</code> field to indicate the path of the directory that you want to monitor.</p>

<p>As the recursive property is set to false, the listener does not monitor the child directories of the main directory that it listens to.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/directory-listener", "_blank");
      }}
...
<p>To run this sample, use the <code>bal run</code> command. After running the sample,</p>

<p>create a new file called <code>test1.txt</code> in the directory called <code>observed-dir</code>, modify it, and delete it.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run directory_listener.bal`}</span>
