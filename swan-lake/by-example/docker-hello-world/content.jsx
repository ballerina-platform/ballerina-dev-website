...
export function DockerHelloWorld({codeSnippets}) {
...
<p>Ballerina supports generating Docker artifacts from code without any additional configuration. This simplifies the experience of developing and deploying Ballerina code in the cloud. Code to Cloud builds the containers and required artifacts by deriving the required values from the code. If you want to override the default values taken by the compiler, you can use a <code>Cloud.toml</code> file.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal build --cloud="docker"`}</span>
<span>{`
`}</span>
...
<p>Verify if the Docker image is generated.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref2}>
      <code className="d-flex flex-column"><span>{`\$ docker images`}</span>
<span>{`
`}</span>
...
<p>Run the generated Docker image.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref3}>
      <code className="d-flex flex-column"><span>{`\$ docker run -d -p 9090:9090 wso2inc/hello:v0.1.0`}</span>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref4}>
      <code className="d-flex flex-column"><span>{`\$ curl http://localhost:9090/helloWorld/sayHello`}</span>
<span>{`Hello from Docker!`}</span>
</code></pre>
          </Col>
        </Row>
