...
export function ErrorBindingPatternInMatchStatement({codeSnippets}) {
...
const ref1 = createRef()
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      <h1>Error binding pattern in match statement</h1>


<p>Error binding patterns can be used in a match statement to bind parts of a successful match of an error value to variables. You can use the rest binding pattern (<code>...r</code>) in the error binding pattern of a match statement to bind the detail mapping fields that are not explicitly bound in the binding pattern.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/error-binding-pattern-in-match-statement", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run error_binding_pattern_in_match_statement.bal`}</span>
<span>{`Matched Generic Error with message:Generic Error and code:20`}</span>
<span>{`Matched DatabaseError with code:2`}</span>
<span>{`Matched NetworkError with message:Bad Request, type:http error, phase:application and elapsedTime:338`}</span>
</code></pre>
          </Col>
        </Row>
...
<ul style={{ marginLeft: "0px" 
      ...
      <Link
        title="RegExp type"
        href="/learn/by-example/regexp-type"
      >
