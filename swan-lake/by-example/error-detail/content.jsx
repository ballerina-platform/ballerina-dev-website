...
<p>Error detail is a map that contains arbitrary extra details about the context of the error. The <code>error&lt;T&gt;</code> type describes an error value with a detail map that has the type <code>T</code>. Named arguments of the error constructor specify the fields of the detail record. An immutable copy of each field is made in the detail record using the <code>cloneReadOnly</code> function.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/error-detail", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run error_detail.bal`}</span>
<span>{`{"httpCode":27}`}</span>
</code></pre>
          </Col>
        </Row>
      ...
      <Link
        title="Error cause"
        href="/learn/by-example/error-cause"
      >
