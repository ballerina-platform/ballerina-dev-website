...
export function ExpressionEquality({ codeSnippets }) {
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      <h1>Expression equality</h1>

      <p>
        Expression equality is determined using two operators: <code>==</code>{" "}
        for value equality and <code>===</code> for reference equality. The{" "}
        <code>==</code> operator checks for deep equality between two values by
        comparing the actual data. In contrast, the <code>===</code> operator
        checks whether two values share the same storage identity, meaning it
        checks if the values reference the same memory location.
      </p>

      <p>
        Values with storage identity, such as structured types like maps and
        arrays, have an identity that comes from the location where the value is
        stored. For such values, the <code>===</code> check determines if two
        references point to the same location. For simple types such as integers
        and booleans, which do not have storage identity, <code>===</code>{" "}
        behaves the same as <code>==</code> except for floating point values.
      </p>
      ...
      >
        <Col className="d-flex align-items-start" sm={12}>
          <button
            ...
            onClick={() => {
              window.open(
                "https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.12.10/examples/expression-equality",
                "_blank",
              );
            }}
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run expression_equality.bal`}</span>
              <span>{`true`}</span>
              <span>{`false`}</span>
              <span>{`false`}</span>
              <span>{`true`}</span>
