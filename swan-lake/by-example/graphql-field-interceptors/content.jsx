...
<p>The GraphQL resolver functions allow adding interceptors for GraphQL requests to execute custom logic. An interceptor can be defined using a <code>readonly</code> class that includes the <code>graphql:Interceptor</code> type. The interceptor class must implement the <code>execute</code> remote method, which is defined in the <code>graphql:Interceptor</code> service object type. It can be passed as a single interceptor or an array of interceptors using the <code>interceptors</code> field in the <code>graphql:ResourceConfig</code> annotation. The provided interceptors will be executed using the <code>_onion principle_</code>. Use the field interceptors to execute custom logic before and after executing a <code>resource</code> or a <code>remote</code> method that needs to be separated from the business logic.</p>


<blockquote>
<p><strong>Note:</strong> A resolver can have zero or more interceptors.</p>
</blockquote>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run graphql_field_interceptors.bal`}</span>
