...
<p>The Ballerina <code>graphql</code> module allows defining GraphQL interface types. An interface specifies a set of fields that multiple object types can include. In Ballerina, interfaces are defined using <code>distinct</code> <code>service</code> objects and the fields of the interfaces are defined as resource method definitions. Objects that are implementing the interfaces must implement the <code>resource</code> methods defined in the <code>service</code> objects. The Ballerina type inclusion includes the interface type to an object type. Use an interface to return a type that consists of a set of possible types with common fields.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/graphql-interfaces", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run graphql_interfaces.bal`}</span>
</code></pre>
          </Col>
        </Row>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref2}>
      <code className="d-flex flex-column"><span>{`\$ curl -X POST -H "Content-type: application/json" -d '{ "query": "{ profiles { name  ...on Teacher { subject }}}" }' 'http://localhost:9090/graphql'`}</span>
...
<ul style={{ marginLeft: "0px" 
                        ...
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="/spec/graphql/#46-interfaces">GraphQL interfaces - Specification</a></span>
                        </li>
