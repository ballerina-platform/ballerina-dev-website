...
export function GraphqlServiceFieldObject({ codeSnippets }) {
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      ...
      <p>
        The Ballerina <code>graphql</code> module exposes information about a
        GraphQL field in a document using the <code>graphql:Field</code> object.
        When the <code>graphql:field</code> is needed to be accessed, define it
        as a parameter of the <code>resource</code>/<code>remote</code> method
        that represents a GraphQL field. Use the <code>graphql:Field</code>{" "}
        object in scenarios where the information about the fields such as
        subfield names and field type are needed for optimizing the business
        logic such as query optimizations.
      </p>

      <blockquote>
        <p>
          <strong>Hint:</strong> The <code>graphql:Field</code> is defined
      ...
      <blockquote>
        <p>
          <strong>Note:</strong> If the <code>graphql:Field</code> is defined as
          a parameter of a resolver function, it will be accessible inside the
          resolver. Passing it down is not necessary.
        </p>
      </blockquote>
      ...
      >
        <Col className="d-flex align-items-start" sm={12}>
          <button
            ...
            onClick={() => {
              window.open(
                "https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.12.10/examples/graphql-service-field-object",
                "_blank",
              );
            }}
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run graphql_service_field_object.bal`}</span>
            </code>
          </pre>
        </Col>
      </Row>
      ...
      >
        ...
        <Col sm={12}>
          <pre ref={ref2}>
            <code className="d-flex flex-column">
              <span>{`\$ curl -X POST -H "Content-type: application/json" -H "scope: admin" -d '{ "query": "{ profile { name age } }" }' 'http://localhost:9090/graphql'`}</span>
      ...
      <p>
        This will print a log message in the server terminal similar to the
        following log verifying that the <code>address</code> field is not
        queried.
      </p>
      ...
      >
        ...
        <Col sm={12}>
          <pre ref={ref3}>
            <code className="d-flex flex-column">
              <span>{`time = 2023-01-27T14:32:47.888+05:30 level = INFO module = "" message = "Address field is not queried"`}</span>
            </code>
          </pre>
        </Col>
      </Row>
      ...
      <p>
        Now, send the following document of which the <code>address</code> field
        is queried.
      </p>
      ...
      >
        ...
        <Col sm={12}>
          <pre ref={ref4}>
            <code className="d-flex flex-column">
              <span>{`\$ curl -X POST -H "Content-type: application/json" -d '{ "query": "{ profile { name address { city } } }" }' 'http://localhost:9090/graphql'`}</span>
      ...
      <p>
        This will print a log message in the server terminal similar to the
        following log verifying that the <code>address</code> field is queried.
      </p>
      ...
      >
        ...
        <Col sm={12}>
          <pre ref={ref5}>
            <code className="d-flex flex-column">
              <span>{`time = 2023-01-27T14:33:47.057+05:30 level = INFO module = "" message = "Querying all the fields"`}</span>
            </code>
          </pre>
        </Col>
      </Row>
