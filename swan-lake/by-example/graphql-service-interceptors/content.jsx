...
export function GraphqlServiceInterceptors({codeSnippets}) {
...
const ref2 = createRef()
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      <h1>GraphQL service - Service interceptors</h1>


<p>The <code>graphql:Service</code> allows adding interceptors for GraphQL requests to execute custom logic. An interceptor can be defined using a <code>readonly</code> class that includes the <code>graphql:Interceptor</code> type. The interceptor class must implement the <code>execute</code> remote method, which is defined in the <code>graphql:Interceptor</code> service object type. They can be passed as an array using the <code>interceptors</code> field in the <code>graphql:ServiceConfig</code> annotation. The provided interceptors will be executed using the <code>_onion principle_</code>. Use the interceptors to execute custom logic before and after executing the <code>resource</code> and <code>remote</code> methods that need to be separated from the business logic.</p>


<blockquote>
<p><strong>Note:</strong> A service can have zero or more interceptors.</p>
</blockquote>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run graphql_interceptors.bal`}</span>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref2}>
      <code className="d-flex flex-column"><span>{`\$ curl -X POST -H "Content-type: application/json" -d '{ "query": "{ name }"}' 'http://localhost:9090/graphql'`}</span>
...
<ul style={{ marginLeft: "0px" 
                        ...
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="https://lib.ballerina.io/ballerina/graphql/latest#Interceptor"><code>graphql:Interceptor</code> object - API documentation</a></span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} class="relatedLinks">
