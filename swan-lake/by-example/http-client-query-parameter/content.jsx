...
<p>The <code>http:Client</code> supports specifying query parameters as resource method arguments. Each query parameter can be stated as a key-value pair. When the request is sent, the key-value pairs are appended to the request path (e.g., <code>?foo=bar</code>). The supported types are <code>string</code>, <code>int</code>, <code>float</code>, <code>boolean</code>, <code>decimal</code>, and <code>array</code> types of the aforementioned types. Use this when invoking endpoints that expect query parameters.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/http-client-query-parameter", "_blank");
      }}
...
<ul style={{ marginLeft: "0px" 
                        ...
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span>Run the HTTP service given in the <a href="/learn/by-example/http-query-parameter/">REST service - Query parameter</a> example.</span>
                        </li>
                    </ul>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run http_client_query_parameter.bal`}</span>
<span>{`Received albums: [{"title":"Blue Train", "artist":"John Coltrane"}]`}</span>
</code></pre>
          </Col>
        </Row>

<p>Furthermore, a <code>post</code> request with a query parameter can be sent as shown below.</p>
