...
<p>Errors that occurred in the request-response flow can be intercepted and handled by the <code>ResponseErrorInterceptors</code>. In addition, the <code>http:RequestErrorInterceptor</code> can be used to handle errors that occurred while executing the <code>RequestInterceptors</code>. This error interceptor can send a response according to the error similar to a <code>ResponseErrorInterceptor</code>. Moreover, it can modify the request and dispatch it to the target service. Use <code>RequestErrorInterceptors</code> along with <code>RequestInterceptors</code> to validate the request beforehand and handle any errors during this validation.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/http-interceptor-error-handling", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run http_interceptor_error_handling.bal`}</span>
</code></pre>
          </Col>
        </Row>
