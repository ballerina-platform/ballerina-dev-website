...
<p>The <code>http:RequestInterceptor</code> is used to intercept the request and execute some custom logic. A <code>RequestInterceptor</code> is a service object with only one resource method, which is executed before dispatching the request to the actual resource in the target service. This resource method can have parameters just like a usual resource method in an <code>http:Service</code>. A <code>RequestInterceptor</code> can be created from a service class, which includes the <code>http:RequestInterceptor</code> service type. Then, this service object can be engaged at the listener level or service level by using the <code>interceptors</code> field in the configurations. This field accepts an array of interceptor service objects as an interceptor pipeline, and the interceptors are executed in the order in which they are placed in the pipeline. Use <code>RequestInterceptors</code> to execute some common logic such as logging, header manipulation, state publishing, etc., for inbound requests.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/http-request-interceptor", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run http_request_interceptor.bal`}</span>
</code></pre>
          </Col>
        </Row>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref2}>
      <code className="d-flex flex-column"><span>{`\$ curl -v http://localhost:9090/albums -H "x-api-version: v2"`}</span>
...
<blockquote>
<p><strong>Tip:</strong> You can invoke the above service via the <a href="/learn/by-example/http-client-send-request-receive-response/">Send request/Receive response client</a> example by adding the required header to the request.</p>
</blockquote>
...
<ul style={{ marginLeft: "0px" 
                        ...
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="/spec/http/#811-request-interceptor">HTTP service request interceptor - Specification</a></span>
                        </li>
