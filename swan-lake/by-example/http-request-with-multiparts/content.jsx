...
export function HttpRequestWithMultiparts({codeSnippets}) {
...
<p>The multipart payload is one or more different sets of data combined in a single body. The <code>http:Client</code> supports multipart content setting and retrieving in the <code>http:Request</code> along with the nested parts through support functions. An array of <code>mime:Entity</code> is returned when retrieving parts through <code>getBodyParts</code> method of the <code>http:Request</code>. If the received parts contain nested parts, you can loop through the parent parts and get the child parts. When sending out multipart content, <code>setBodyParts</code> is used to set the array of <code>mime:Entity</code>. This is useful to handle different content-typed messages as a single payload and large payloads.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/http-request-with-multiparts", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run request_with_multiparts.bal`}</span>
...
<p>In the directory, which contains the <code>.bal</code> file, create a directory named <code>files</code>, and add an XML file named <code>test.xml</code> in it.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref2}>
      <code className="d-flex flex-column"><span>{`\$ curl -F "part1={\\"name\\":\\"ballerina\\"};type=application/json" http://localhost:9090/multiparts/decoder -H "Content-Type: multipart/mixed" -H 'Expect:'`}</span>
...
<p>Execute the following cURL command to encode the parts of the body and send a multipart request via the Ballerina service.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref3}>
      <code className="d-flex flex-column"><span>{`\$ curl -v http://localhost:9090/multiparts/encoder`}</span>
...
<ul style={{ marginLeft: "0px" 
                        ...
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="https://lib.ballerina.io/ballerina/http/latest#Request#setBodyParts"><code>setBodyParts()</code> - API documentation</a></span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} class="relatedLinks">
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="/spec/mime/#3-supported-multipart-types">HTTP client supported-multipart-types - Specification</a></span>
                        </li>
      ...
      <Link
        title="Chunking"
        href="/learn/by-example/http-client-chunking"
      >
