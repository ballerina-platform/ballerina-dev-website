...
export function HttpResponseInterceptor({codeSnippets}) {
...
<p>The <code>http:ResponseInterceptor</code> is used to intercept the response and execute some custom logic. A <code>ResponseInterceptor</code> is a service object with a remote method called <code>interceptResponse</code>, which is executed before dispatching the response to the client. A <code>ResponseInterceptor</code> can be created from a service class, which includes the <code>http:ResponseInterceptor</code> service type. Then, this service object can be engaged at the listener level or service level by using the <code>interceptors</code> field in the configurations. This field accepts an array of interceptor service objects as an interceptor pipeline, and the interceptors are executed in the order in which they are placed in the pipeline. Use <code>ResponseInterceptors</code> to execute some common logic such as logging, header manipulation, state publishing, etc., for all outbound responses.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/http-response-interceptor", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run http_response_interceptor.bal`}</span>
</code></pre>
          </Col>
        </Row>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref2}>
      <code className="d-flex flex-column"><span>{`\$ curl -v http://localhost:9090/albums`}</span>
...
<blockquote>
<p><strong>Tip:</strong> You can invoke the above service via the <a href="/learn/by-example/http-client-send-request-receive-response/">Send request/Receive response client</a>.</p>
</blockquote>
...
<ul style={{ marginLeft: "0px" 
                        ...
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="/spec/http/#812-response-interceptor">HTTP service response interceptor - Specification</a></span>
                        </li>
