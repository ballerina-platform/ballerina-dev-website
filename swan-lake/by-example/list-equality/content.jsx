...
<p>You can use <code>==</code> and <code>!=</code> on lists to check the deep equality of two lists: two lists are deep equal if they have the same members in the same order. Deep equality only works for <code>anydata</code> lists. <code>===</code> and <code>!==</code> check for the exact equality, which matches the references of the lists.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/list-equality", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run list_equality.bal`}</span>
<span>{`true`}</span>
<span>{`false`}</span>
<span>{`true`}</span>
<span>{`true`}</span>
