...
<p>A <code>class</code> defines an object type and provides a way to construct an object value.  An object can be constructed from the <code>class</code> definition using the implicit and explicit <code>new</code> expression. The type of the constructor should be mentioned after <code>new</code> for the explicit new expression. For the implicit new expression, the type will be inferred from the expected type.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/object-value-from-class-definition", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run object_value_from_class_definition.bal`}</span>
<span>{`Null`}</span>
<span>{`Alice`}</span>
<span>{`Bob`}</span>
<span>{`Null`}</span>
