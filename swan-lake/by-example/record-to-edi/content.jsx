...
export function RecordToEdi({ codeSnippets }) {
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      ...
      <p>
        The same prebuilt package used in the EDI to record conversion example
        serializes in the other direction: build the typed record for the
        message and call <code>toEdiString</code> to get EDI text for an
        outbound document.
      </p>

      <p>
        <code>toEdiString</code> writes the message body. Wrap it in an envelope
        with <code>interchangeToEdiString</code>, which emits the{" "}
        <code>UNB</code>/<code>UNH</code> headers and recomputes the{" "}
        <code>UNT</code> and <code>UNZ</code> counts, when a full interchange is
        needed.
      </p>
      ...
      >
        <Col className="d-flex align-items-start" sm={12}>
          <button
            ...
            onClick={() => {
              window.open(
                "https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.13.5/examples/record-to-edi",
                "_blank",
              );
            }}
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run record_to_edi.bal`}</span>
              <span>{`BGM+220+PO20001+9'`}</span>
              <span>{`DTM+137:20260821:102'`}</span>
              <span>{`RFF+CT:CONTRACT-77'`}</span>
              <span>{`NAD+BY+SUPERMART'`}</span>
              <span>{`NAD+SU+SUPPLIER456'`}</span>
              <span>{`UNS+S'`}</span>
            </code>
          </pre>
        </Col>
      ...
      <p>
        The output is the same message that the EDI to record conversion example
        reads back, so the two examples form a round trip.
      </p>
