...
export function RegexpType({codeSnippets}) {
...
<p><code>RegExp</code> is a built-in type in Ballerina that represents a regular expression. Regular expressions are commonly used for tasks such as validating input, searching and replacing text, and parsing data.</p>


<p>The <code>RegExp</code> type is defined in the <code>lang.regexp</code> module, and can also be referenced using the type alias named the same in the <code>lang.string</code> module. The <code>RegExp</code> type conforms to a subset of the ECMAScript specification for regular expressions.</p>


<p>A <code>RegExp</code> value can be created by using the regexp template expression or calling the <code>fromString</code> method of the <a href="https://lib.ballerina.io/ballerina/lang.regexp/latest#fromString">lang.regexp</a> module.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/regexp-type", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run regexp_type.bal`}</span>
<span>{`Pattern \`HELLO*\` matches \`HELLO\`: true`}</span>
<span>{`Pattern \`a+\` matches \`aaa\`: true`}</span>
<span>{`Pattern \`abc|\${"xyz"}\` matches \`xyz\`: true`}</span>
<span>{`Pattern \`\\p{Ll}\` matches \`a\`: true`}</span>
<span>{`Pattern \`\\p{P}\` matches \`0\`: true`}</span>
<span>{`Pattern \`\\p{sc=Latin}\` matches \`aεЛ\`: a`}</span>
<span>{`Pattern \`(?i:BalleRINA)\` matches \`Ballerina\`: true`}</span>
</code></pre>
          </Col>
        </Row>
...
<ul style={{ marginLeft: "0px" 
                        ...
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="https://ballerina.io/spec/lang/master/#section_10.1">Ballerina regular expression grammar</a></span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} class="relatedLinks">
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="/learn/by-example/regexp-operations">RegExp langlib functions</a></span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} class="relatedLinks">
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="https://lib.ballerina.io/ballerina/lang.regexp">RegExp API Docs</a></span>
                        </li>
                    </ul>
<ul style={{ marginLeft: "0px" 
                      }} class="relatedLinks">
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="https://lib.ballerina.io/ballerina/lang.string">string API Docs</a></span>
                        </li>
