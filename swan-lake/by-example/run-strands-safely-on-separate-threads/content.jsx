...
export function RunStrandsSafelyOnSeparateThreads({ codeSnippets }) {
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      <h1>Run strands safely on a separate thread based on isolation</h1>

      <p>
        The isolated feature in Ballerina supports identifying cases in which
        strands created by a start action, or a named worker can be run safely
        on separate threads.
      </p>

      <p>
        A <code>start</code> action is isolated if the function or method it
        calls has a type that is isolated and the expression for every argument
        is an isolated expression. The object whose method is called by the
        method call or remote method call is treated as an argument. An isolated{" "}
        <code>start</code> action is allowed in an isolated function and the
        strand created by the <code>start</code> action will run on a separate
        thread from the current thread.
      </p>

      <p>
        The strand created by a named worker can run on a separate thread from
        the default worker if the body of the worker satisfies the requirements
        for an isolated function.
      </p>
      ...
      >
        <Col className="d-flex align-items-start" sm={12}>
          <button
            ...
            onClick={() => {
              window.open(
                "vscode://wso2.ballerina/open-file?repoFileUrl=https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.8.3/examples/run-strands-safely-on-separate-threads/run_strands_safely_on_separate_threads.md",
                "_blank",
              );
            }}
            target="_blank"
          ...
          <button
            ...
            onClick={() => {
              window.open(
                "https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.8.3/examples/run-strands-safely-on-separate-threads",
                "_blank",
              );
            }}
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run_strands_safely_on_separate_threads.bal`}</span>
              <span>{`true`}</span>
              <span>{`true`}</span>
              <span>{`true`}</span>
            </code>
