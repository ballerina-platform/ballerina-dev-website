  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      ...
      <p>
        ...
        <code>ftp:Listener</code>. An <code>ftp:Listener</code> with SFTP
        protocol is created by providing the protocol, host-name, required
        credentials, and the private key. The directory each service watches is
        given by the <code>@ftp:ServiceConfig</code> annotation. Once connected,
        the listener polls that directory and dispatches every file it finds to
        the service. The handler is selected by the file extension and the
        content is bound to its first parameter, so <code>onFileText</code>{" "}
        receives the file as a string and the handler never reads it.{" "}
        <code>onFileJson</code>, <code>onFileXml</code>, and{" "}
      ...
      >
        <Col className="d-flex align-items-start" sm={12}>
          <button
            ...
            onClick={() => {
              window.open(
                "https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.13.5/examples/sftp-service-receive-file",
                "_blank",
              );
            }}
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run sftp_service_receive_file.bal`}</span>
      ...
      <blockquote>
        <p>
          ...
          <code>/home/in</code> on the SFTP server to trigger the service.
        </p>
      </blockquote>
      ...
      <Row className="mt-auto mb-5">
        ...
        <Col sm={6}>
          <Link title="Caller object" href="/learn/by-example/sftp-caller/">
