...
export function SmbCaller({ codeSnippets }) {
  ...
  return (
    <Container className="bbeBody d-flex flex-column h-100">
      <h1>SMB service - Caller object</h1>

      <p>
        An SMB service reacts to the files that arrive in the directory it
        watches. To act on the share while handling one of those files, the
        service declares an <code>smb:Caller</code> parameter on the handler.
        The <code>smb:Caller</code> is the share connection the listener already
        holds, so no second connection is opened. It offers the same operations
        as the <code>smb:Client</code>: <code>putText</code>,{" "}
        <code>putJson</code>, <code>putXml</code>, <code>putCsv</code>, and{" "}
        <code>putBytes</code> to write, the matching <code>get</code> methods to
        read, and <code>move</code>, <code>copy</code>, <code>rename</code>, and{" "}
        <code>delete</code> to manage files. Use this to produce a result on the
        remote file system for each file that arrives.
      </p>
      ...
      <ul style={{ marginLeft: "0px" }}>
        <li>
          ...
          <span>
            ...
            <code>/sales/new</code> and <code>/sales/ack</code> directories. The
            image creates no users, so add <code>user1</code> with the password{" "}
            <code>pass456</code> in the <code>WORKGROUP</code> domain.
          </span>
        </li>
      </ul>

      <p>
        Run the program by executing the following command. An acknowledgement
        file is written to <code>/sales/ack</code> for each new file in the
        watched directory.
      </p>
        ...
        <Col sm={12}>
          <pre ref={ref1}>
            <code className="d-flex flex-column">
              <span>{`\$ bal run smb_caller.bal`}</span>
              <span>{`Acknowledged store-14.json`}</span>
            </code>
          </pre>
        </Col>
      </Row>
      ...
      <ul style={{ marginLeft: "0px" }} class="relatedLinks">
        <li>
          <span>&#8226;&nbsp;</span>
          <span>
            <a href="https://lib.ballerina.io/ballerina/smb/latest#Caller">
              <code>smb:Caller</code> client object - API documentation
            </a>
          </span>
        </li>
      </ul>
