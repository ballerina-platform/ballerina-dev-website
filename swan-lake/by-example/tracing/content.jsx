...
<p>Ballerina supports Observability out of the box, and Tracing is one of the three important aspects of observability. To observe Ballerina code, the <code>--observability-included</code> build time flag should be given along with the <code>Config.toml</code> file when starting the service. The <code>Config.toml</code> file contains the required runtime configurations related to observability.</p>


<p>You can trace the code blocks and measure the time incurred during the actual runtime execution. Also, you can choose to hook the measurement with the default trace created or can create a completely new trace.</p>


<blockquote>
<p><strong>Info:</strong> For more information about configs and observing applications, see <a href="/learn/observe-ballerina-programs/">Observe Ballerina programs</a>.</p>
</blockquote>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.5.0/examples/tracing", "_blank");
      }}
...
<p>Invoke the service using cURL and access <a href="http://localhost:16686">Jaeger UI</a>.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ curl http://localhost:9234/hello/sayHello`}</span>
<span>{`Hello, World!`}</span>
</code></pre>
          </Col>
        </Row>

<p>Jaeger is the default tracing tool used in Ballerina. To start the Jaeger, execute the command below.</p>


<p>To start the service, navigate to the directory that contains the</p>

<p><code>.bal</code> file, and execute the <code>bal run</code> command below with the <code>--observability-included</code> build time flag and the <code>Config.toml</code> runtime configuration file.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref2}>
      <code className="d-flex flex-column"><span>{`\$ docker run -d -p 13133:13133 -p 16686:16686 -p 55680:55680 jaegertracing/opentelemetry-all-in-one`}</span>
