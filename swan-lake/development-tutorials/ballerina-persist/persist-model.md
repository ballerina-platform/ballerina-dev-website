...
### Declare custom DB schema with `Schema` annotation

The `@sql:Schema` annotation can be used to declare a custom schema for the table. This annotation can only be used on the entity record type.
...
The `Person` entity will be mapped to the `my_schema.person` table in the database. This annotation can only be used with MSSQL and PostgreSQL data stores.
