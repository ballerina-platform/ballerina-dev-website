...
Once you build the Ballerina package, the compiler extension will generate the artifacts required for openshift deployment.

>**Tip:** If you don't have the cloud="openshift" entry on the `Ballerina.toml` you can execute `bal build --cloud=openshift` command to provide the build option inline.
...
>**Note:** Before invoking the Kubernetes service, observe the Openshift yaml that have been generated. You should be able to find services for ports that have been exposed, `HorizontalPodAutoscaler` for scaling and Deployment, and Config maps.
