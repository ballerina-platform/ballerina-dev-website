...
You can specify code generation tools to integrate with the package build. These tools execute before the package build and generate code used during the build process.
...
The mandatory `id` field specifies a unique identifier for the tool entry, as there can be multiple entries for the same tool command to generate code for different modules. The `id` must consist of alphanumeric characters and underscores only, but it must not begin or end with an underscore, and consecutive underscores are not permitted.
...
The `options` fields are defined by the tool developer and used to pass additional parameters to the tool.

If a tool contains multiple subcommands, you can specify them as follows.
