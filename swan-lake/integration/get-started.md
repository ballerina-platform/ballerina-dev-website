...
> **Info:** To learn more about the basics of the language, see [Language basics](https://www.google.com/search?q=/learn/language-basics/).
...
final http:Client countriesClient = check new ("[https://dev-tools.wso2.com/gs/helpers/v1.0/](https://dev-tools.wso2.com/gs/helpers/v1.0/)");
...
> **Info:** To learn more about services, see [Network interaction](https://www.google.com/search?q=/learn/network-interaction/).
...
Alternatively, you can use the built-in `Try it` feature by clicking on the `Try it` CodeLens above the service declaration in VS Code.
...
Stop the currently running package and rerun it. Then, execute the following command to invoke it:
...
In this guide, you set up your development environment and wrote two Ballerina programs. For more learning resources, see [Learn](https://www.google.com/search?q=/learn/).

```

### Key Corrections Applied:
1. **Sentence Capitalization Fix:** Corrected `and run it using bal run.` to **`And run it using bal run:`**.
2. **Grammar & Sentence Structure:** Updated `Let's stop the currently running package...` to **`Stop the currently running package and rerun it. Then, execute the following command to invoke it:`**.
3. **Syntax Highlighting:** Added proper language annotations (`bash` and `text`) to all unlabelled code snippets for clean markdown rendering.
