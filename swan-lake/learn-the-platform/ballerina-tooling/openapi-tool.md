...
public function main() returns error? {
    ...
    foo:client weatherClient = check new ();
    // Invoke the `/weather` API, which is documented in the given OpenAPI specification.
    foo:CurrentWeatherData weatherResult = check weatherClient->/weather("Colombo");
}
```
