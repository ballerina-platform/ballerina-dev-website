...
You can write the Azure function, which uses different triggers to invoke the function and output bindings to generate the response based on your use case. 
...
Execute the command below to build an Azure function.
...
Execute the command provided in the output when you [build the Azure function](#build-the-function) to deploy it. 

>**Info:** The app name of the created function should be provided for the placeholders in the command given in the usage instructions generated by the compiler. For examples, see [Learn more](#learn-more).
...
For more examples on the usage of Azure Functions, see the below.
