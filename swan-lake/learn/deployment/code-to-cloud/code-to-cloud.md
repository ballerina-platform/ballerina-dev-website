...
Code to cloud builds the containers and required artifacts by deriving the required values from the code. This process happens when the project is being compiled. In order to override the default values given by the compiler, the `Ballerina.cloud` file needs to be created in the project directory. 

## Project Structure

Ballerina encourages to have one microservice per project. In order to adhere to that rule, code to cloud generates only one container per project. These artifacts can be found in the `target` directory of the project. A complete representation of the project structure is as follows.
...
>**Note:** The `Ballerina.cloud` file is completely optional and you only have to specify the values you need to override. If the value is not specified for a certain field, the compiler will take the default value. All the supported properties can be found [below](#properties-of-the-ballerina-.-cloud-file).
...
## Properties of the 'Ballerina.cloud' File
